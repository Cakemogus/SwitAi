...
async def ask_switai(prompt: str) -> str:
    ...
    if re.search(r"вердикт", prompt, re.IGNORECASE):
        ...
        judge_prompt = (
            f"Ты — жёсткий швейцарский аналитик. Сейчас в РП {current_month}. Учитывай это при анализе. "
            f"Разбери следующий текст. Если это экономика — укажи конкретные риски (отказ партнёров, санкции, конкуренты). "
